--https://github.com/BanMing/BanMingLuaLab/blob/master/LuaTest/Test/LuaJson.lua(来源，如有侵权，请联系删除)
Json = {}
...
local function json2array(str, from, to)
    ...
    while (pos <= to) do
        ...
        elseif char == '{' then
            result[#result + 1], pos = Json.json2table(str, pos, to)
        elseif char == ']' then
            return result, pos
...
local function table2json(key, value)
    ...
    local tableStr = Json.table2json(value)
    return string.format("\"%s\":%s,", key, tableStr)
end



function Json.json2table(str, from, to)
    local result = {}
    from = from or 1
    local pos = from + 1
    local to = to or string.len(str)
    ...
    while (pos <= to) do
        ...
        elseif char == '{' then
            if not key then
                key, pos = Json.json2table(str, pos, to)
            else
                result[key], pos = Json.json2table(str, pos, to)
                key = nil
            end
        elseif char == '}' then
            return result, pos
